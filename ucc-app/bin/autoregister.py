...
try:
    ...
    # Importable outside splunkd (the signatures-file generator and its lockstep
    # test import TOOLS/build_tool_docs from here; only the handler needs the base).
...
def build_tool_docs():
    """The tool definitions, one dict per tool, in mcp_tools KV-record shape.

    Single source of truth for BOTH registration paths:
      - Enterprise: _register_kv upserts these into the MCP Server KV (below).
      - Cloud: the SAME docs are shipped as appserver/static/
        tool_input_payload_signatures.json (generated by
        scripts/gen_tool_signatures.py), which the native synced-apps registrar
        reads on install. Without that file Cloud registers every tool with an
        empty inputSchema and no execution body, so a client sees a parameterless
        tool it cannot call. tests/test_tool_signatures.py asserts the shipped
        file stays equal to this.
    """
    return [_doc(*t) for t in TOOLS]


def _register_kv(sk):
    out = []
    for doc in build_tool_docs():
        tid = doc["tool_id"]
        advertised = mcp_name(doc["name"])
